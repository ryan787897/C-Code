...
public final class RSSAggregator {
    ...
    private static void processFeed(String url, String file, SimpleWriter out) {
        ...
        outputFooter(out); //outputs footer
    }
    ...
    private static void processItem(XMLTree item, SimpleWriter out) {
        ...
        if (tableDate >= 0 && item.child(tableDate).numberOfChildren() > 0) {
        ...
        if (tableSource >= 0
                && item.child(tableSource).numberOfChildren() > 0) {
        ...
        if (tableTitle >= 0 && item.child(tableTitle).numberOfChildren() > 0) {
    ...
    public static void main(String[] args) {
        ...
        SimpleWriter feedOut = new SimpleWriter1L(fileName);

        String title = xml.attributeValue("title");

        feedOut.println("<html><h1>" + title + "</h1></html>");

        for (int i = 0; i < xml.numberOfChildren(); i++) {
            String urlN = xml.child(i).attributeValue("url");
            String ur1 = xml.child(i).attributeValue("name");
            String fileN = xml.child(i).attributeValue("file");
            SimpleWriter fileOut1 = new SimpleWriter1L(fileN);
            processFeed(urlN, fileN, fileOut1);
            String U1 = "<a href=\"" + fileN + "\">" + ur1 + "</a>";
            feedOut.println("<ul><li>" + U1 + "</li></ul>");
            fileOut1.close();
        }
